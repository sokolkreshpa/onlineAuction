...
const apiSearchUrl = 'api/_search/bids';

// Actions

export const searchEntities = createAsyncThunk('bid/search_entity', async ({ query, page, size, sort }: IQueryParams) => {
...
export const updateEntity = createAsyncThunk(
  'bid/update_entity',
  async (entity: IBid, thunkAPI) => {
    const result = await axios.put<IBid>(`${apiUrl}/${entity.id}`, cleanEntity(entity));
    thunkAPI.dispatch(getEntities({}));
    return result;
  },
  { serializeError: serializeAxiosError }
...
export const partialUpdateEntity = createAsyncThunk(
  ...
  async (entity: IBid, thunkAPI) => {
    const result = await axios.patch<IBid>(`${apiUrl}/${entity.id}`, cleanEntity(entity));
    thunkAPI.dispatch(getEntities({}));
    return result;
  },
  { serializeError: serializeAxiosError }
...
export const deleteEntity = createAsyncThunk(
  ...
  async (id: string | number, thunkAPI) => {
    const requestUrl = `${apiUrl}/${id}`;
    const result = await axios.delete<IBid>(requestUrl);
    thunkAPI.dispatch(getEntities({}));
    return result;
  },
  { serializeError: serializeAxiosError }
