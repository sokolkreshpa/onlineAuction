...
import com.someg.auction.config.EmbeddedKafka;
import com.someg.auction.config.KafkaSseConsumer;
import com.someg.auction.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class OnlineAuctionKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/online-auction-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/online-auction-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/online-auction-kafka/unregister"));
                return;
            }
        }
